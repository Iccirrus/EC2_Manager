  ...
  try:
    ...
    for res in reservations:
      for inst in res.instances:
        ...
        start_wknd = inst.tags['start:wknd'] if 'start:wknd' in inst.tags else None
        stop_wknd = inst.tags['stop:wknd'] if 'stop:wknd' in inst.tags else None
        ...
        # queue up instances that have the start time falls between now and the next 30 minutes on a weekday
        if start_sched != None and state == "stopped" and weekday != 5 and weekday != 6 and time_to_action(start_sched, now, 31 * 60):
          start_list.append(inst.id)

        # queue up instances that have the stop time falls between 30 minutes ago and now on a weekday
        if stop_sched != None and state == "running" and weekday != 5 and weekday !=6 and time_to_action(stop_sched, now, 31 * -60):
          stop_list.append(inst.id)
          
        # queue up instances that have the start time falls between now and the next 30 minutes on a weekend
        if start_wknd != None and state == "stopped" and (weekday = 5 or weekday = 6) and time_to_action(start_sched, now, 31 * 60):
          start_list.append(inst.id)
          
       # queue up instances that have the stop time falls between now and the next 30 minutes on a weekend
        if start_wknd != None and state == "stopped" and (weekday = 5 or weekday = 6) and time_to_action(start_sched, now, 31 * 60):
           stop_list.append(inst.id)
